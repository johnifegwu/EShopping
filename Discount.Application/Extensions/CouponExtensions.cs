...
namespace Discount.Application.Extensions
{
    public static class CouponExtensions
    {
        ...
        public static async Task<bool> IsCouponExist(this UpdateCouponRequest coupon, IUnitOfWorkCore _unitOfWork)
        {
            var result = await Task.FromResult(_unitOfWork.Repository<Coupon>().Read().Where(x => x.ProductId == coupon.ProductId).FirstOrDefault());
        ...
        public static async Task<bool> IsCouponExist(this CreateCouponRequest coupon, IUnitOfWorkCore _unitOfWork)
        {
            var result = await Task.FromResult(_unitOfWork.Repository<Coupon>().Read().Where(x => x.ProductId == coupon.ProductId).FirstOrDefault());
